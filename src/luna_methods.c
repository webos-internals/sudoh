...
// Called directly from webOS, and returns directly to webOS.
//
bool sudo_method(LSHandle* lshandle, LSMessage *message, void *ctx) {
  LSError lserror;
  LSErrorInit(&lserror);
  ...
  json_t *command = json_find_first_label(object, "command");               
  if (!command || (command->child->type != JSON_STRING)) {
  ...
  sprintf(reply, "{\"returnValue\": true, \"reply\": \"sudo %s\"}", command->child->text);
